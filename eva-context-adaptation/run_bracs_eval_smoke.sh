# =============================================================================
# BRACS Evaluation Smoke Test - Fair Comparison
# =============================================================================
#
...
# Output directories
OUTPUT_BASE=${OUTPUT_BASE:-/home/paul/OpenMidnight/outputs/bracs_eval_smoke}
...
EVAL_CONFIG=${EVAL_CONFIG:-/home/paul/OpenMidnight/eval_configs/bracs_embeddings_only.yaml}
...
EVAL_SCRIPT="${SCRIPT_DIR}/bracs_context_eval.py"

echo "=============================================="
echo "BRACS Evaluation Smoke Test"
echo "=============================================="
echo "Samples per split: ${N_SAMPLES}"
echo "Data backend:      ${DATA_BACKEND}"
echo "BRACS root:        ${BRACS_ROOT}"
echo "AWS Bucket:        ${AWS_BUCKET}"
echo "AWS BRACS Root:    ${AWS_BRACS_ROOT}"
...
if [ "${SKIP_BASELINE}" = "true" ]; then
    ...
    echo "    This uses the exact patches from BRACS."
    echo ""

    python "${EVAL_SCRIPT}" \
        --mode baseline \
        --data-backend "${DATA_BACKEND}" \
        --bracs-root "${BRACS_ROOT}" \
        --baseline-checkpoint "${BASELINE_CKPT}" \
        --aws-endpoint "${AWS_ENDPOINT}" \
        --aws-profile "${AWS_PROFILE}" \
        --aws-bucket "${AWS_BUCKET}" \
        --aws-bracs-root "${AWS_BRACS_ROOT}" \
